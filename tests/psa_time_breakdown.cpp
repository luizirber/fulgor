...
int do_timed_map(FulgorIndex const& index, fastx_parser::FastxParser<fastx_parser::ReadSeq>& rparser,
    ...
    essentials::timer<std::chrono::high_resolution_clock, std::chrono::nanoseconds> lookup_timer, deduplication_timer, intersection_timer;
    vector<essentials::timer<std::chrono::high_resolution_clock, std::chrono::nanoseconds>> timers = {lookup_timer, deduplication_timer, intersection_timer};
            ...
            for (auto const& record : rg) {
                switch (algo) {
                    case pseudoalignment_algorithm::FULL_INTERSECTION:
                        index.pseudoalign_full_intersection_timed(record.seq, colors, timers);
                        break;
    ...
    std::cout << "Lookup took: " << timers[0].elapsed() << " nanoseconds (" << timers[0].elapsed() / 1000000000 << " seconds)" << std::endl;
    std::cout << "Deduplication took: " << timers[1].elapsed() << " nanoseconds (" << timers[1].elapsed() / 1000000000 << " seconds)" << std::endl;
    std::cout << "Intersection took: " << timers[2].elapsed() << " nanoseconds (" << timers[2].elapsed() / 1000000000 << " seconds)" << std::endl;
...
template <typename FulgorIndex>
int pseudoalign_timed(std::string const& index_filename, std::string const& query_filename,
    ...
    for (uint64_t i = 1; i != num_threads; ++i) {
        workers.push_back(std::thread([&index, &rparser, &num_reads, &num_mapped_reads, algo,
            ...
            do_timed_map(index, rparser, num_reads, num_mapped_reads, algo, threshold, out_file, iomut,
                   ofile_mut);
        }));
    }
...
int psa_time_breakdown(int argc, char** argv) {
    /* params */
    ...
    if (sshash::util::ends_with(index_filename,
        ...
        return pseudoalign_timed<meta_differential_index_type>(
        ...
        return pseudoalign_timed<meta_index_type>(index_filename, query_filename, output_filename,
                                            num_threads, threshold, algo, quiet);
    } else if (sshash::util::ends_with(index_filename,
                                       constants::diff_colored_fulgor_filename_extension)) {
        return pseudoalign_timed<differential_index_type>(index_filename, query_filename, output_filename,
                                                    num_threads, threshold, algo, quiet);
    } else if (sshash::util::ends_with(index_filename, constants::fulgor_filename_extension)) {
        return pseudoalign_timed<index_type>(index_filename, query_filename, output_filename, num_threads,
